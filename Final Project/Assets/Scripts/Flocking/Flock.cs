...
public class Flock : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        LayerMask lm = LayerMask.GetMask(mask);
        for (int i = 0; i < numberOfBoids; i++)
        {
            ...
            //ShootTree s = boids[i].GetComponent<ShootTree>();
            //ShootTree.SetValue("Mask", lm);
        }
    }
        ...
        if (deadBoids.Count != 0)
        {
            foreach (GameObject g in deadBoids)
            {
                ...
                if (g != null)
                {
                    ...
                    //done
                    Instantiate(boom, g.transform.position, Quaternion.identity);
